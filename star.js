var starjs = {
  correct : function(message) {
    // Credit:
    // http://stackoverflow.com/questions/11919065/sort-an-array-by-the-levenshtein-distance-with-best-performance-in-javascript/11958496#11958496
    ...
    function isUpperCase(text) { return (text[0].toUpperCase() == text[0]); }
    ...
    if ((wordsOfMessage.length == 1) && (correction.length == 1) &&
        (wordsOfMessage[0] == correction[0])) {
      return false;
    }
  ...
  closeKeys : function(inputText, correction, radius = null) {
    ...
    // it is an optional param, but it reduces the search space if set to less
    // than 1.
    // reducing the search space can increase correction accuracy, at the
    // expense of requiring a
    ...
    // it's up to the algorthim to determine whether a match is "too good" (i.e
    // the ranking is too high and to find a slightly lower one.) This is
    // because if the ranking is too high, it means that the input and
    // correction are identical, and it's unlikely that the user corrected a
    // perfectly good word.

    // returns an array, the first element is the ranking of how closely the
    // inputText could resemble the output from 0 to length of inputText or
    // correction, whichever is greater. The higher the number the higher
    // probability that it could be spelled with the correction.
    // the second element is the same thing as the first, except the possible
    // values range from 0 to 1, 0 being completely unlikely it could be
    // generated from the correction, and 1 being 100% chance it was (i.e is the
    // same text.)
    ...
    for (var j = 0; j < correction.length; j++) {
      if (closeLetters[j] !== undefined) {
        ...
        // if it is the first letter (i.e the same letter) set to one, which is
        // the highest ranking
        if (ranking === 0) {
          ranking = 1;
        }
        ...
        // something bad happened, and the correction/inputText is too long or
        // short.
      }
    }
    ...
    return [
      totalRanking,
      ((totalRanking / Math.max(correction.length, inputText.length)) + 1) / 2
    ];
  }
